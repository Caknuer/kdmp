...
@section('title', 'Edit Unit Bisnis')
@section('page_title', 'Edit Unit Bisnis')
@section('page_subtitle', 'Perbarui informasi unit bisnis: ' . $businessUnit->name)

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <form action="{{ route('admin.business-units.update', $businessUnit) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select id="category_select"
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat }}" {{ old('category', $businessUnit->category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                        @endforeach
                        @if(!in_array($businessUnit->category, $categories->toArray()))
                            <option value="{{ $businessUnit->category }}" selected>{{ $businessUnit->category }}</option>
                        @endif
                ...
                <div>
                    ...
                    <input type="number"
                           id="order"
                           name="order"
                           value="{{ old('order', $businessUnit->order) }}"
                           min="0"
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center gap-3">
                    <div id="iconPreview" class="w-11 h-11 rounded-lg bg-gray-100 border border-gray-300 flex items-center justify-center text-gray-700 text-lg flex-shrink-0">
                        <i class="fas fa-{{ old('icon', $businessUnit->icon ?: 'building') }}"></i>
                    </div>
                    <input type="text"
                           id="icon"
                           name="icon"
                           value="{{ old('icon', $businessUnit->icon) }}"
            ...
            <div class="mb-6">
                <label for="thumbnail" class="block text-sm font-semibold text-gray-700 mb-2">
                    Thumbnail / Foto Unit Bisnis
                </label>
                <div class="flex items-start gap-4">
                    <div id="thumbPreviewContainer" class="w-24 h-24 rounded-xl bg-gray-100 border border-gray-300 flex items-center justify-center overflow-hidden flex-shrink-0 text-gray-400">
                        @if($businessUnit->thumbnail_url)
                            <img src="{{ $businessUnit->thumbnail_url }}" alt="{{ $businessUnit->name }}" class="w-full h-full object-cover">
                        @else
                            <i class="fas fa-{{ $businessUnit->icon ?: 'building' }} text-3xl text-gray-400"></i>
                        @endif
                    </div>
                    <div class="flex-1">
                        <input type="file"
                        ...
                        <p class="text-xs text-gray-500 mt-1">Biarkan kosong jika tidak ingin mengubah gambar thumbnail saat ini.</p>
                        @if($businessUnit->thumbnail)
                            <label class="inline-flex items-center gap-2 mt-2 text-xs text-red-600 cursor-pointer">
                                ...
                                <span>Hapus foto thumbnail saat ini</span>
                            </label>
                        @endif
                    </div>
                </div>
            ...
            <div class="mb-6">
                ...
                <textarea id="services"
                          ...
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500">{{ old('services', $businessUnit->services) }}</textarea>
                @error('services')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea id="description"
                          ...
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500">{{ old('description', $businessUnit->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-save"></i> Perbarui Unit Bisnis
                    </button>
                </div>
            </div>
        </form>
