...
@section('page_title', 'Edit Data Pengawas')
@section('page_subtitle', 'Perbarui data, jabatan, foto, atau status dewan pengawas')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <form action="{{ route('admin.pengawas.update', $pengawas) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text"
                           name="name_p"
                           id="name_p"
                           value="{{ old('name_p', $pengawas->name_p) }}"
                           required
                ...
                <div>
                    <label for="role" class="block text-sm font-semibold text-gray-700 mb-2">
                        Jabatan Pengawasan <span class="text-red-500">*</span>
                    </label>
                    <input type="text"
                           name="role"
                           id="role"
                           value="{{ old('role', $pengawas->role) }}"
                           required
                ...
                <div>
                    ...
                    <input type="number"
                           name="order"
                           id="order"
                           value="{{ old('order', $pengawas->order) }}"
                           min="0"
            ...
            <div class="mb-6">
                ...
                <div class="flex items-start gap-4">
                    <div id="photoPreviewContainer" class="w-20 h-20 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                        @if($pengawas->photo_url)
                            <img src="{{ $pengawas->photo_url }}" alt="{{ $pengawas->name_p }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full bg-purple-100 text-purple-700 font-bold flex items-center justify-center text-xl">
                                {{ strtoupper(substr($pengawas->name_p, 0, 1)) }}
                            </div>
                        @endif
                    </div>
                    <div class="flex-1">
            ...
            <div class="mb-6">
                ...
                <textarea name="bio"
                          id="bio"
                          rows="4"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500">{{ old('bio', $pengawas->bio) }}</textarea>
                @error('bio')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox"
                           name="is_active"
                           value="1"
                           {{ old('is_active', $pengawas->is_active) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Jika dicentang, pengawas akan ditampilkan di halaman publik KDMP.</p>
                    </div>
                </label>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-save"></i> Perbarui Pengawas
                </button>
            </div>
        </form>
    </div>
