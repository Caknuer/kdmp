...
@section('page_title', 'Edit Anggota Organisasi')
@section('page_subtitle', 'Perbarui informasi anggota organisasi')

@section('content')
<div class="bg-white rounded-lg shadow-md p-8">
    <form action="{{ route('admin.profile.members.update', $member) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <input type="text"
                   name="name_p"
                   id="name_p"
                   value="{{ old('name_p', $member->name_p) }}"
        ...
        <div class="mb-6">
            ...
            <select name="type"
                ...
                <option value="pengurus" {{ old('type', $member->type) == 'pengurus' ? 'selected' : '' }}>Pengurus</option>
                <option value="pengawas" {{ old('type', $member->type) == 'pengawas' ? 'selected' : '' }}>Pengawas</option>
            </select>
            @error('type')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <p class="text-sm text-gray-500">Biarkan kosong jika tidak ingin mengubah foto.</p>
            </div>
        </div>
        @endif
        ...
        <div class="mb-6">
            <label for="photo_p" class="block text-sm font-semibold text-gray-700 mb-2">
                {{ $member->photo_p ? 'Ganti Foto' : 'Foto Profil' }}
            </label>
            <input type="file"
                   name="photo_p"
                   id="photo_p"
        ...
        <div class="mb-6">
            ...
            <textarea name="bio"
                      ...
                      rows="4"
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500"
                      placeholder="Deskripsikan singkat tentang anggota ini...">{{ old('bio', $member->bio) }}</textarea>
            @error('bio')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
