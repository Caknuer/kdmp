...
@section('page_title', 'Kelola Pengurus KDMP')
@section('page_subtitle', 'Kelola data pengurus, jabatan, foto, dan urutan tampilan struktur pengurus')
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 flex items-center justify-between">
        <div>
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Pengurus</p>
            <p class="text-2xl font-bold text-gray-800 mt-1">{{ $pengurus->total() }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 flex items-center justify-between">
        <div>
            ...
            <a href="{{ url('/profil/pengurus') }}" target="_blank" class="inline-flex items-center gap-2 text-sm font-semibold text-rose-600 hover:text-rose-700 mt-2">
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 mb-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <form action="{{ route('admin.pengurus.index') }}" method="GET" class="flex flex-wrap items-center gap-3 flex-1">
            ...
            @if(request('search') || request('status') && request('status') != 'all')
                <a href="{{ route('admin.pengurus.index') }}" class="text-sm text-gray-500 hover:text-gray-700 underline">
                    Reset
                </a>
            @endif
        </form>
                ...
                <tr class="hover:bg-gray-50/80 transition">
                    ...
                    <td class="px-6 py-4">
                        @if($item->photo_url)
                            ...
                            <div class="w-12 h-12 rounded-full bg-blue-100 text-blue-700 font-bold items-center justify-center hidden text-base">
                                {{ strtoupper(substr($item->name_p, 0, 1)) }}
                            </div>
                        @else
                            <div class="w-12 h-12 rounded-full bg-blue-100 text-blue-700 font-bold flex items-center justify-center text-base">
                                {{ strtoupper(substr($item->name_p, 0, 1)) }}
                            </div>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 text-gray-700 font-medium">
                        <span class="inline-block px-3 py-1 bg-blue-50 text-blue-800 rounded-md font-medium text-xs border border-blue-100">
                            {{ $item->role }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <form action="{{ route('admin.pengurus.destroy', $item) }}"
                                  method="POST"
                                  onsubmit="return confirm('Apakah Anda yakin ingin menghapus data pengurus ini?')"
                                  class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                        <div class="max-w-sm mx-auto">
                            ...
                            <h4 class="font-semibold text-gray-800 text-base mb-1">Belum Ada Data Pengurus</h4>
                            <p class="text-sm text-gray-500 mb-4">Tambahkan data pengurus untuk ditampilkan di profil KDMP.</p>
                            <a href="{{ route('admin.pengurus.create') }}" class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-sm font-semibold transition inline-flex items-center gap-2">
                                <i class="fas fa-plus"></i> Tambah Pengurus
                            </a>
                        </div>
                    </td>
