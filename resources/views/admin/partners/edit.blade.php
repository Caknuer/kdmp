...
@section('page_subtitle', 'Edit informasi mitra: ' . $partner->name)
        ...
        <form action="{{ route('admin.partners.update', $partner) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent"
                          placeholder="Deskripsikan tentang mitra ini...">{{ old('description', $partner->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="text-sm text-gray-600">
                        <p>Ganti logo dengan mengupload file baru di bawah ini.</p>
                        <p>File saat ini: {{ basename($partner->logo) }}</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="mb-6">
                <label for="logo" class="block text-sm font-semibold text-gray-700 mb-2">
                    {{ $partner->logo ? 'Ganti Logo' : 'Logo Mitra' }}
                </label>
                <input type="file" id="logo" name="logo" accept="image/*"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent">
                <p class="text-xs text-gray-500 mt-1">Format: PNG, JPG, JPEG, SVG. Maksimal 2MB. {{ $partner->logo ? 'Biarkan kosong jika tidak ingin mengganti logo.' : '' }}</p>
                @error('logo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex gap-3">
                <button type="submit" class="px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg transition font-semibold">
                    <i class="fas fa-save mr-2"></i> Update Mitra
                </button>
                <a href="{{ route('admin.partners.show', $partner) }}" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition font-semibold">
