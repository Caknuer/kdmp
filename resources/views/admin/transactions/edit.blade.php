...
<div class="bg-white rounded-lg shadow-md p-8 max-w-2xl">
    <form action="{{ route('admin.transactions.update', $transaction) }}" method="POST" class="space-y-6">
        ...
        <input type="hidden" name="transaction_for" id="transaction_for" value="{{ old('transaction_for', $transaction->transaction_for) }}">
        ...
        <div id="cash_section" class="hidden">
            ...
            <input list="categories-cash" name="category" value="{{ old('category', $transaction->category) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    selectTransactionType('{{ old('transaction_for', $transaction->transaction_for) }}');
});
</script>
@endpush
